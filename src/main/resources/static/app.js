...
class App extends React.Component {
	...
	loadFromServer(pageSize) {
		follow(client, root, [
			...
			this.setState({
				contacts: employeeCollection.entity._embedded.contacts,
	...
	onCreate(newEmployee) {
		follow(client, root, ['contacts']).then(employeeCollection => {
			return client({
				method: 'POST',
	...
	// tag::delete[]
	onDelete(contact) {
		client({method: 'DELETE', path: contact._links.self.href}).done(response => {
...
class EmployeeList extends React.Component {
	...
	// tag::contact-list-render[]
	render() {
		var contacts = this.props.contacts.map(contact =>
			<Contact key={contact._links.self.href} contact={contact} onDelete={this.props.onDelete}/>
		);
